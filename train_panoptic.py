...
def prepare_panoptic_data(args):
    ...
    num_train_imgs = len(train_images) # to be used to set number of iterations

    logger.info("--- Finished Full Dataset Preparation ---")
    return instance_thing_id_map, semantic_stuff_id_map, global_panoptic_map, cat_id_to_name, void_class, num_train_imgs

def train_main(args):
    """ 
    This is the function that will be distributed across GPUs.
    It receives 'args' which contains the computed 'data_maps'.
    """
    setup_logger(output=args.output_dir)
    
    # Unpack the maps calculated in the main process
    instance_thing_id_map, semantic_stuff_id_map, global_panoptic_map, cat_id_to_name, void_class, num_train_imgs = args.data_maps
    # Register datasets
    train_dataset_name = f"{args.dataset_name}_panoptic_train"
    val_dataset_name = f"{args.dataset_name}_panoptic_val"

    # Create the metadata components to be returned
    # print(instance_thing_id_map, semantic_stuff_id_map)
    metadata = {
        ...
        "thing_dataset_id_to_contiguous_id": {i+1 : i for i in range(len(instance_thing_id_map.keys()))},  
        "stuff_dataset_id_to_contiguous_id": {cat_id : i+1 for i, cat_id in enumerate(semantic_stuff_id_map.keys())},
    }
    # print({i+1 : i for i in range(len(instance_thing_id_map.keys()))})
    # print({cat_id : i+1 for i, cat_id in enumerate(semantic_stuff_id_map.keys())})
    # metadata = {}
    ...
    cfg.MODEL.ROI_HEADS.BATCH_SIZE_PER_IMAGE = 128  # Pick that much random objects from FPN proposal. 128 for fast, 512 for accurate.

    cfg.DATALOADER.NUM_WORKERS = 2  # CPU per GPU to load data. Increase if utilizaiton is low.

    cfg.SOLVER.IMS_PER_BATCH = args.ims_per_batch_per_gpu*args.num_gpus  # Increase if VRAM is not used. R101 FPN normally 2 imgs to fill 8 GB
    # Set iter according to IMS_PER_BATCH, num_epochs (how many times to go over the dataset)
    one_epoch_iters = num_train_imgs / cfg.SOLVER.IMS_PER_BATCH
    cfg.SOLVER.MAX_ITER = int(args.num_epochs * one_epoch_iters)
    
    cfg.SOLVER.BASE_LR = 0.0001*args.num_gpus   # scale with gpu, we can jump higher. Standard.

    cfg.SOLVER.CLIP_GRADIENTS.ENABLED = True
    cfg.OUTPUT_DIR = args.output_dir

    logger.info("Configuration complete:" \
    f"NUM_CLASSES_THINGS: {cfg.MODEL.ROI_HEADS.NUM_CLASSES}\n" \
    f"NUM_CLASSES_STUFF: {cfg.MODEL.SEM_SEG_HEAD.NUM_CLASSES}\n" \
    f"NUM_WORKERS: {cfg.DATALOADER.NUM_WORKERS}\n" \
    f"BATCH_SIZE_PER_IMAGE: {cfg.MODEL.ROI_HEADS.BATCH_SIZE_PER_IMAGE}\n" \
    f"NUM_EPOCHS: {args.num_epochs}\n" \
    f"NUM_GPUS: {args.num_gpus}\n" \
    f"IMS_PER_BATCH: {cfg.SOLVER.IMS_PER_BATCH}\n" \
    f"NUM_TRAIN_IMGS: {num_train_imgs}\n" \
    f"MAX_ITER: {cfg.SOLVER.MAX_ITER}\n" \
    f"BASE_LR: {cfg.SOLVER.BASE_LR}\n")
    ...
    if args.eval_only:
        ...
        evaluator = COCOPanopticEvaluator(val_dataset_name + "_separated", output_dir=os.path.join(cfg.OUTPUT_DIR, "panoptic_evaluation"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset-name", type=str, default="snow_4cam", help="The base name of the dataset files")
    parser.add_argument("--eval-only", action="store_true", help="Run evaluation on the validation set")
    parser.add_argument("--ims-per-batch-per-gpu", type=int, default=3, help="Number of images per batch per gpu. Set that so that VRAM is not overflown.")
    parser.add_argument("--num-epochs", type=int, default=500, help="Number of epochs, going over the dataset full cycle.")
    ...
    # 1. RUN DATA PREPARATION ONCE (Main Process)
    # We capture the maps and attach them to args
    data_maps = prepare_panoptic_data(args)
    args.data_maps = data_maps 

    # 2. LAUNCH MULTI-GPU TRAINING
    # launch() will spawn 'num_gpus' processes running 'train_main(args)'
    logger.info(f"Launching training on {args.num_gpus} GPUs...")
    launch(
        train_main,
